                        ...
                        <div class="col-md-3">
                            ...
                            <div class="verification-actions mb-3">
                                <button type="button" class="btn btn-success btn-lg w-100 mb-2"
                                        onclick="showVerifyModal({{ $driver->id }}, '{{ $driver->full_name ?? $driver->first_name }}')">
        ...
        <div class="modal-content">
            ...
            <form method="POST" id="verifyForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="verify_admin_password" class="form-label">Admin Password <span class="text-danger">*</span></label>
                        <input type="password" class="form-control" name="admin_password" id="verify_admin_password" 
                               placeholder="Enter your admin password to confirm" required>
                        <small class="form-text text-muted">This confirms your authorization to verify drivers.</small>
                    </div>
                    
                    <div class="mb-3">
                        <label for="verify_verification_notes" class="form-label">Verification Notes</label>
                        <textarea class="form-control" name="verification_notes" id="verify_verification_notes" 
            ...
            <form method="POST" id="rejectForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="rejection_notes" id="rejection_notes" 
                                  rows="4" placeholder="Provide specific details about why this driver was rejected..."></textarea>
                    </div>
                    
                    <div class="mb-3">
                        <label for="reject_admin_password" class="form-label">Admin Password <span class="text-danger">*</span></label>
                        <input type="password" class="form-control" name="admin_password" id="reject_admin_password" 
                               placeholder="Enter your admin password to confirm" required>
                        <small class="form-text text-muted">This confirms your authorization to reject drivers.</small>
                    </div>
                </div>
...
function showVerifyModal(driverId, driverName) {
    $('#verifyDriverName').val(driverName);
    $('#verifyForm').attr('action', `/admin/drivers/${driverId}/verify`);
    $('#verify_admin_password').val(''); // Clear password field
    $('#verify_verification_notes').val(''); // Clear notes field
...
function showRejectModal(driverId, driverName) {
    ...
    $('#reject_admin_password').val(''); // Clear password field
...
$('#verifyForm').on('submit', function(e) {
    ...
    $.post(form.attr('action'), form.serialize())
        ...
        .fail(function(xhr) {
            let errorMessage = 'Error verifying driver';
            if (xhr.responseJSON && xhr.responseJSON.errors) {
                if (xhr.responseJSON.errors.admin_password) {
...
$('#rejectForm').on('submit', function(e) {
    ...
    $.post(form.attr('action'), form.serialize())
        ...
        .fail(function(xhr) {
            let errorMessage = 'Error rejecting driver';
            if (xhr.responseJSON && xhr.responseJSON.errors) {
                if (xhr.responseJSON.errors.admin_password) {
                    errorMessage = 'Invalid admin password';
                } else {
